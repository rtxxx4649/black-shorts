...
async function videosApi(request) {
  try {
    ...
    if (!youtubeApiKey) return Response.json({ regionCode, count: videos.length, videos: videos.map(v => ({ ...v, channel_id: "", channel_avatar: "" })) });
    ...
    if (channelIds.length) {
      ...
      for (const channel of channelData.items || []) channelMap.set(channel.id, { channel_avatar: channel.snippet?.thumbnails?.high?.url || channel.snippet?.thumbnails?.medium?.url || channel.snippet?.thumbnails?.default?.url || "" });
    }
    ...
    const enriched = videos.map(video => {
      ...
      return { ...video, views: statistics.viewCount ?? video.views ?? 0, likes: statistics.likeCount ?? video.likes ?? 0, comments: statistics.commentCount ?? video.comments ?? 0, channel_id: channelId, channel_avatar: channel.channel_avatar || "" };
    });
